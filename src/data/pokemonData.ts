// Pokemon data organized by generation for wheel system
// Contains all Pokemon from Gen 1-9 (Pokedex #1-1025)
...
export const pokemonGenerations: PokemonGeneration[] = [
  ...
  },
  {
    ...
    pokemon: [
      ...
      'Rabsca', 'Flittle', 'Espathra', 'Tinkatink', 'Tinkaton', 'Tinkaton', 'Wiglett',
...
// Utility function to get all Pokemon from selected generations
export const getAllPokemonFromGenerations = (selectedGenerations: number[]): string[] => {
  const allPokemon: string[] = [];
  
  selectedGenerations.forEach(genIndex => {
    if (pokemonGenerations[genIndex]) {
      allPokemon.push(...pokemonGenerations[genIndex].pokemon);
    }
  });
...
// Get all Pokemon from all generations (default)
export const getAllPokemon = (): string[] => {
  return getAllPokemonFromGenerations([0, 1, 2, 3, 4, 5, 6, 7, 8]); // All 9 generations
};

// Utility function to format Pokemon names for image URLs
export const formatPokemonNameForImage = (pokemonName: string): string => {
  return pokemonName
    .toLowerCase()
    .replace(/\s+/g, '-')       // Replace spaces with hyphens
    .replace(/['.♀♂:]/g, '')   // Remove special characters
    .replace(/farfetchd/g, 'farfetchd')  // Handle special cases
    .replace(/mr-mime/g, 'mr-mime')
    .replace(/mime-jr/g, 'mime-jr')
    .replace(/type-null/g, 'type-null')
    .replace(/tapu-/g, 'tapu-')
    .replace(/jangmo-o/g, 'jangmo-o')
    .replace(/hakamo-o/g, 'hakamo-o')
    .replace(/kommo-o/g, 'kommo-o')
    .replace(/wo-chien/g, 'wo-chien')
    .replace(/chien-pao/g, 'chien-pao')
    .replace(/ting-lu/g, 'ting-lu')
    .replace(/chi-yu/g, 'chi-yu');
};

// Generate Pokemon artwork URL for fusion wheel (pokemondb.net)
export const getPokemonArtworkUrl = (pokemonName: string): string => {
  const formattedName = formatPokemonNameForImage(pokemonName);
  return `https://img.pokemondb.net/artwork/${formattedName}.jpg`;
};

// Generate Pokemon image URL for custom Pokemon wheel (Cloudinary)
export const getCustomPokemonImageUrl = (pokemonId: string): string => {
  // Using the Cloudinary configuration 
  return `https://res.cloudinary.com/dpxlbcjql/image/upload/v1/pokemon-images/${pokemonId}.jpg`;
};

// Legacy function - keeping for backward compatibility
export const getPokemonImageUrl = (pokemonName: string): string => {
  return getPokemonArtworkUrl(pokemonName);
};